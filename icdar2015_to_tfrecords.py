...
if __name__ == "__main__":
    root_dir = util.io.get_absolute_path('~/Desktop/PhD_projects/pixel_link-master/datasets/ICDAR2017/')
    output_dir = util.io.get_absolute_path('~/Desktop/PhD_projects/pixel_link-master/datasets/TF/')
    util.io.mkdir(output_dir);

    training_data_dir = util.io.join_path(root_dir, 'train/train_img')
    training_gt_dir = util.io.join_path(root_dir,'train/train_gt')
    cvt_to_tfrecords(output_path = util.io.join_path(output_dir, 'icdar2017_train.tfrecord'), data_path = training_data_dir, gt_path = training_gt_dir)

    test_data_dir = util.io.join_path(root_dir, 'test/test_img')
    test_gt_dir = util.io.join_path(root_dir,'test/test_gt')
    cvt_to_tfrecords(output_path = util.io.join_path(output_dir, 'icdar2017_test.tfrecord'), data_path = test_data_dir, gt_path = test_gt_dir)
